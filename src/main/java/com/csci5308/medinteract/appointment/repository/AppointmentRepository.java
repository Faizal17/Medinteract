...
//    @Query("SELECT a,p,d FROM AppointmentModel a JOIN DoctorModel d ON d.id = a.doctorId JOIN PatientModel p ON p.id = a.patientId WHERE DATE(a.startTime) = DATE(?1) AND a.isActive = true")
//    List<Object> findDailyAppointments(LocalDateTime localDate);
